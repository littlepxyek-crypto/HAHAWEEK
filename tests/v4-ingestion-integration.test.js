...
test('IngestionEngine uses V4 cursor adapter only when explicitly supplied', async()=>{
  ...
  const adapter={get:()=>100,advance:(n)=>{calls.push(n);return n;}};
  const engine=new IngestionEngine({provider:provider(102),cursor:{get:()=>999,advance:()=>{throw new Error('LEGACY_CURSOR_USED');}},confirmations:0,processor:async b=>processed.push(b),v4CursorAdapter:adapter});
...
test('IngestionEngine does not advance V4 cursor after processor failure', async()=>{
  const calls=[]; const adapter={get:()=>100,advance:n=>calls.push(n)};
...
test('IngestionEngine refuses implicit V4 initialization', async()=>{
  const adapter={get:()=>null,advance:()=>{}};
