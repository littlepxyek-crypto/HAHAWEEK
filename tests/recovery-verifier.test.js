...
const {
  ...
  verifyCursorRecord,
  verifyRecovery,
} = require("../src/reference/v4/recovery-verifier");
const vectors = require("../docs/golden-vectors/checkpoint-cursor-recovery.json");

function vector(id) {
  const found = vectors.vectors.find((v) => v.id === id);
...
test("offline verifier accepts the valid checkpoint record", () => {
  const v = vector("checkpoint-genesis-valid");
  const result = verifyCheckpointRecord(
    { input: v.input, hash: "0x" + v.expected_hash },
    v.manifest,
  );
  assert.equal(result.hash, "0x" + v.expected_hash);
});
...
test("offline verifier accepts the valid cursor record", () => {
  const v = vector("cursor-genesis-valid");
  const result = verifyCursorRecord(
    { input: v.input, hash: "0x" + v.expected_hash },
    v.checkpoint,
  );
  assert.equal(result.hash, "0x" + v.expected_hash);
...
test("offline verifier rejects checkpoint generation mismatch", () => {
  ...
  assert.throws(
    () => verifyCheckpoint(v.input, { ...v.manifest, generation: "8" }),
    /generation mismatch/,
  );
});

test("offline verifier rejects a mutated checkpoint digest", () => {
  const v = vector("checkpoint-genesis-valid");
  assert.throws(
    () => verifyCheckpointRecord({
      input: v.input,
...
test("offline verifier rejects a mutated cursor digest", () => {
  const v = vector("cursor-genesis-valid");
  assert.throws(
    () => verifyCursorRecord({
      input: v.input,
