...
test('F-03 invalid range and invalid digest fail closed before mutation', async () => {
  ...
  assert.throws(
    () => readF03AuthorityChain({ database, fromBlock: 110, toBlock: 109 }),
    /F03_RANGE_INVALID/
  );
  ...
  const invalid = {
    ...
    segment: {
      ...fixture.segment,
      segmentDigest: 'Z'.repeat(64),
    },
  };

  assert.throws(
    () => commitF03AuthorityChain({ database, writerFence, ...invalid }),
...
test('F-03 generation and linkage conflicts fail closed', async () => {
  ...
  const generationConflict = {
    ...
    manifest: {
      ...
      provenance: provenance({
        recordType: 'manifest',
        recordId: fixture.manifest.manifestId,
        table: 'f03_manifests',
        key: fixture.manifest.manifestId,
        artifactId: fixture.manifest.manifestId,
        upstreamRecordId: fixture.segment.segmentId,
        generation: '2',
        integrityDigest: fixture.manifest.manifestDigest,
        committedAt: fixture.manifest.committedAt,
      }),
    },
  };
  ...
  const linkageConflict = {
    ...
    manifest: {
      ...
      provenance: provenance({
        ...
        key: fixture.manifest.manifestId,
        artifactId: fixture.manifest.manifestId,
        upstreamRecordId: 'other-segment',
...
test('F-03 missing segment and failed transaction leave no authoritative chain', async () => {
  const dir = tempDir();
  const { database, writerFence } = await createFileDatabase(dir);
  const fixture = chainFixture();
  ...
  assert.throws(
    () => commitF03AuthorityChain({
      ...
      checkpoint: { ...fixture.checkpoint, manifestDigest: 'Z'.repeat(64) },
    }),
  ...
  const restart = await createFileDatabase(dir);
  assert.throws(
    () => readF03AuthorityChain({ database: restart.database, fromBlock: 100, toBlock: 109 }),
    /F03_CHAIN_NOT_FOUND/
  );
...
test('F-03 schema 3 migration rolls back when required table creation conflicts', async () => {
  const dir = tempDir();
  const file = path.join(dir, 'migration-rollback.sqlite');
  const first = await createFileDatabase(dir, 'migration-rollback.sqlite');
  ...
  await assert.rejects(
    () => createDatabase(file, { legacyWriteBarrier: authority.barrier }),
    /F03_TABLE_MISSING|SQLITE|already exists/
  );
  authority.writerFence.release();
