...
function assertInventory(inventory) {
  ...
  if (JSON.stringify(inventory) !== JSON.stringify(sorted)) fail("inventory must be deterministically sorted");
}

function classifyCommittedFixtures(repoRoot) {
  const directory = path.join(repoRoot, "docs", "golden-vectors");
  if (!fs.existsSync(directory) || !fs.statSync(directory).isDirectory()) fail("golden-vector directory is missing");
  const names = fs.readdirSync(directory).filter((name) => name.endsWith(".json")).sort();
  const knownOutOfScope = new Set(KNOWN_OUT_OF_SCOPE_FIXTURES.map((fixture) => path.posix.basename(fixture)));
  const discoveredV4 = [];

  for (const name of names) {
    const relative = path.posix.join("docs/golden-vectors", name);
    ...
    catch { fail(`unreadable or malformed fixture: ${relative}`); }
    if (document && document.format === V4_FORMAT && document.protocol === V4_PROTOCOL && document.canonicalization === V4_CANONICALIZATION) {
...
function verifyCoverage(options = {}) {
  ...
  if (JSON.stringify(discovered) !== JSON.stringify(expected)) fail(`V4 fixture inventory mismatch: expected [${expected.join(", ")}], discovered [${discovered.join(", ")}]`);
  const results = inventory.map((relative) => {
    const absolute = path.join(repoRoot, relative);
    if (!fs.existsSync(absolute) || !fs.statSync(absolute).isFile()) fail(`missing V4 fixture: ${relative}`);
    const result = verifyVectorSet(absolute);
    return { fixture: relative, vector_count: result.count, status: "VERIFIED" };
  });
  return { status: "VERIFIED", repository_root: repoRoot, fixture_count: results.length, vector_count: results.reduce((total, result) => total + result.vector_count, 0), fixtures: results };
}

if (require.main === module) {
  const result = verifyCoverage();
  process.stdout.write(`verified ${result.fixture_count} V4 golden-vector fixtures / ${result.vector_count} vectors\\n`);
  for (const fixture of result.fixtures) process.stdout.write(`VERIFIED ${fixture.vector_count} ${fixture.fixture}\\n`);
}

module.exports = { V4_GOLDEN_VECTOR_FIXTURES, KNOWN_OUT_OF_SCOPE_FIXTURES, verifyCoverage, classifyCommittedFixtures };
