...
function assertCursorAuthority(cursor, checkpoint) {
  ...
  if (
    !input ||
    Object.keys(input).sort().join(',') !== 'checkpoint_hash,generation,position'
  ) {
    throw new Error('CURSOR_AUTHORITY_INVALID');
  }

  if (
    !validUint64(input.generation) ||
...
function authorizeCursorAdvance({
  ...
  if (BigInt(String(targetPosition)) < BigInt(cursor.input.position)) {
