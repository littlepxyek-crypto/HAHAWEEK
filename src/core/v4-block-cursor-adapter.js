...
const { blockNumberToPosition, assertPositionMatchesBlock } = require('./v4-robinhood-block-position');
...
class V4BlockCursorAdapter {
  ...
  buildNext(blockNumber) {
    ...
    assertPositionMatchesBlock(this.authorityRecord.cursor_input.position, currentPosition);
    const targetPosition = blockNumberToPosition(blockNumber);
