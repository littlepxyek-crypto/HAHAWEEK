...
function assertUint64(value, field) {
  if (typeof value !== "string" || !UINT64_RE.test(value)) {
    invalid(field + " must be canonical uint64 decimal");
  }
  if (BigInt(value) > UINT64_MAX) {
    invalid(field + " exceeds uint64 range");
  }
}

function assertHash(value, field) {
  if (typeof value !== "string" || !HASH32_RE.test(value)) {
    invalid(field + " must be canonical lowercase 0x-prefixed 32-byte hash");
  }
}
...
function exactKeys(value, expected, label) {
  ...
  if (actual.length !== wanted.length || actual.some((key, i) => key !== wanted[i])) {
    invalid(label + " has non-canonical key set");
  }
}

function verifyCheckpoint(checkpoint, manifest) {
  exactKeys(checkpoint, ["generation", "manifest_hash"], "checkpoint");
  if (typeof checkpoint.generation !== "string" || typeof checkpoint.manifest_hash !== "string") {
    invalid("checkpoint values must be strings");
  }
  ...
  if (manifest.inventory_valid !== true || manifest.segments_valid !== true) {
    invalid("manifest inventory or segments are not verified");
  }
...
function verifyCheckpointRecord(record, manifest) {
  ...
  if (record.hash !== verified.hash) {
    invalid("checkpoint digest mismatch");
  }
  return verified;
}
...
function verifyCursor(cursor, checkpoint) {
  ...
  for (const key of ["generation", "checkpoint_hash", "position"]) {
    if (typeof cursor[key] !== "string") invalid("cursor." + key + " must be a string");
  }
  ...
  if (BigInt(cursor.generation) > BigInt(checkpoint.generation)) {
    invalid("cursor generation exceeds checkpoint generation");
  }
...
function verifyCursorRecord(record, checkpoint) {
  ...
  if (record.hash !== verified.hash) {
    invalid("cursor digest mismatch");
  }
  return verified;
}
...
function verifyRecovery(input) {
  if (input === null || typeof input !== "object" || Array.isArray(input)) {
    invalid("recovery input must be an object");
  }
  exactKeys(input, ["manifest", "checkpoint", "cursor", "acquisition_position_valid"], "recovery input");
  if (input.acquisition_position_valid !== true) {
    invalid("acquisition position is not verified");
  }
